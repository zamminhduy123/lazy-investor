...
export function AIBriefingCard({ selectedSymbol }: AIBriefingCardProps) {
  ...
  const latestStatus =
    events.filter((e) => e.type === "status").slice(-1)[0]?.data?.message || "";
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <span
                  className={cn(
                    ...
                    String(performanceData?.["1W"]).includes("-")
                      ? "text-rose-600"
                      : "text-emerald-600",
                  )}
                >
                  {performanceData?.["1W"]} (W)
                </span>
                <span
                  className={cn(
                    "text-[10px] font-mono font-bold",
                    String(performanceData?.["1M"]).includes("-")
                      ? "text-rose-600"
                      : "text-emerald-600",
                  )}
                >
                  {performanceData?.["1M"]} (M)
                </span>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mb-4 bg-white border border-slate-200 rounded-xl p-1 shadow-sm">
          <TabsList className="bg-transparent gap-1">
            {finalResult && <TabsTrigger
              value="summary"
              className="data-[state=active]:bg-slate-900 data-[state=active]:text-white rounded-lg text-xs font-bold uppercase tracking-widest h-9"
            >
          ...
          {finalResult && (
            <Button
              ...
              className="h-8 w-8 text-slate-400 hover:text-slate-600 mr-1"
            >
            ...
            {finalResult ? (
              <>
                ...
                <div className="p-8">
                  <div className="flex items-center gap-3 mb-6">
                    <Badge
                      variant="outline"
                      className="rounded-md bg-slate-50 text-slate-500 border-slate-100 font-mono text-[10px]"
                    >
                      CONFIDENCE:{" "}
                      {currentAnalysis?.overall_summary?.confidence_score || 0}
                      /10
                    </Badge>
                    <span
                      className={cn(
                        "text-xs font-bold px-3 py-1 rounded-full border flex items-center",
                        getSentimentStyles(
                          currentAnalysis?.overall_summary?.market_sentiment ||
                            "",
                        ),
                      )}
                    >
                  ...
                  <p className="text-slate-600 leading-relaxed text-lg mb-8">
                    {currentAnalysis?.overall_summary?.summary ||
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                          ...
                          <span
                            className={cn(
                              "text-xs font-mono font-bold",
                              String(performanceData?.["3M"]).includes("-")
                                ? "text-rose-400"
                                : "text-emerald-400",
                            )}
                          >
                        ...
                        <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                          ...
                          <span
                            className={cn(
                              "text-xs font-mono font-bold",
                              String(performanceData?.["6M"]).includes("-")
                                ? "text-rose-400"
                                : "text-emerald-400",
                            )}
                          >
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span
                            className={cn(
                              "text-xs font-mono font-bold",
                              String(performanceData?.["1Y"]).includes("-")
                                ? "text-rose-400"
                                : "text-emerald-400",
                            )}
                          >
              ...
              <div className="p-12 text-center">
                <p className="text-slate-400">
                  Generate AI analysis in the Deep Dive tab to see summary.
                </p>
              </div>
            )}
          </motion.div>
                            ...
                            <div className="p-5 space-y-6 overflow-hidden">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-3 min-w-0">
                                  <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">
                                    AI Summary
                                  </h5>
                                  <p className="text-sm text-slate-700 leading-relaxed italic border-l-2 border-slate-200 pl-3">
                                    "
                                    {article.analysis?.tldr ||
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                                <div className="space-y-3  min-w-0">
                                  <h5 className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter flex items-center">
                                    <TrendingUp className="w-3 h-3 mr-2" /> Key
                                    Drivers
                                  </h5>
                                  <ul className="space-y-2">
                                    {article.analysis?.key_drivers.map(
                                      (d, i) => (
                                        <li
                                          key={i}
                                ...
                                <div className="space-y-3 min-w-0">
                                  ...
                                  <ul className="space-y-2">
                                    {article.analysis?.risks_or_caveats.map(
                                      (r, i) => (
                                        <li
                                          key={i}
                                          className="text-xs text-slate-600 flex items-start gap-2"
                              ...
                              <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                                <p className="text-[10px] text-slate-400 italic">
                                  {article.analysis?.relevance_reason ||
            ...
            <div className="overflow-auto flex-1 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
              <table className="w-full text-left border-collapse relative">
                <thead className="sticky top-0 z-10 shadow-sm">
                  <tr className="bg-slate-100/95 backdrop-blur-sm border-b border-slate-200">
                    <th className="px-3 py-3 text-[10px] font-black text-slate-500 uppercase tracking-tighter">
                      Ex-Date
                    </th>
                    <th className="px-3 py-3 text-[10px] font-black text-slate-500 uppercase tracking-tighter">
                      Description
                    </th>
                    <th className="py-3 text-[10px] font-black text-slate-500 uppercase tracking-tighter text-center">
                      Type
                    </th>
                    <th className="px-3 py-3 text-[10px] font-black text-slate-500 uppercase tracking-tighter text-right">
                      Value/Ratio
                    </th>
                    <th className="px-3 py-3 text-[10px] font-black text-slate-500 uppercase tracking-tighter text-right">
                      Yield
                ...
                <tbody className="divide-y divide-slate-50 bg-white overflow-x-hidden">
                  {(dividendHistory || [])?.map((div: any, i: number) => (
                    <tr
                      key={i}
                      className="hover:bg-slate-50/50 transition-colors group"
                    >
                      <td className="px-3 py-4 text-xs font-mono font-bold text-slate-600">
                        {div.date}
                      </td>
                      ...
                      <td className="px-3 py-4 text-center">
                        <span className="text-xs font-mono font-bold text-slate-700">
                          {div.type === "CASH"
                            ...
                            : `${(div.ratio * 100).toFixed(0)}%`}
                        </span>
                      </td>
                      <td className="px-3 py-4 text-right">
                        <span className="text-xs font-mono font-bold text-emerald-600">
                          {div.yield_percent
                            ? `${(div.yield_percent / 100).toFixed(2)}%`
                            : "—"}
                        </span>
                      </td>
                    </tr>
              ...
              {(!dividendHistory || dividendHistory.length === 0) && (
                <div className="p-12 text-center text-slate-400 text-sm">
                  No dividend data available for {selectedSymbol} in this
                  prototype.
                </div>
              )}
            </div>
